            ...
            if dificultat in [1, 2, 3, 4]:
                if dificultat == 4:
                    try:
                        ...
                        var_globals.level, var_globals.mida = int(dades[0]), int(dades[1])
                        elements_destapats = eval(dades[2])
                        var_globals.max_animals, var_globals.jugador_vida = int(dades[3]), int(dades[4])
                        var_globals.jugador_x, var_globals.jugador_y = int(dades[5]), int(dades[6])
                        var_globals.caselles_destapades, var_globals.comptador_animals = eval(dades[7]), int(dades[8])
                ...
                while not seguir:
                    ...
                    try:
                        ...
                        if eleccio in [1, 2, 3]:
                            ...
                            print("\n" * 15 + "Error: Opció no vàlida")
                    except ValueError:
                        missatge_error = "Error: Has d'introduir un número" if entrada else "Error: No has introduït res"
                        print("\n" * 15 + missatge_error)
